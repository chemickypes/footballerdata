...
## 1. Divulgativo
Questo modulo è la faccia visibile del progetto: il server Flask che mette insieme asta live, listone, copilot, audit, trade machine e strumenti di supporto stagionale. È pensato per funzionare sia in modalità community sia in modalità personale, adattando configurazione, persistenza e esperienza d’uso al contesto. In pratica, tutto quello che l’utente tocca passa da qui.
...
Tra gli endpoint principali ci sono `/api/players`, `/api/state`, `/api/assign`, `/api/undo`, `/api/ai_query`, `/api/live/snapshot`, oltre agli endpoint per auth e settings. La web app importa i moduli di dominio da `modules/` e la parte AI da `core.copilot`.

## 3. Screenshot
![Command Center Overview](../docs/assets/command_center_overview.png)

## 4. Dipendenze
- Dipende da `core/config.py`, `core/copilot/`, `modules/common/`, `modules/lineup/`, `modules/valuation/`, `modules/trades/`.
